...
import { motion, useInView, useReducedMotion, useSpring, useTransform, type MotionValue } from 'framer-motion';
import { useRef, type ReactNode } from 'react';

import { useConstellationSlideContext } from './constellation-slide-context';

type ConstellationSceneCardProps = {
  children: ReactNode;
  className?: string;
...
export function ConstellationSceneCard({
  children,
  className = '',
  variant = 'section',
  sequence = 'standard',
  delay = 0,
}: ConstellationSceneCardProps) {
...
function SlideSceneCard({
  ...
  delay = 0,
  scrollYProgress,
}: ConstellationSceneCardProps & { scrollYProgress: MotionValue<number> }) {
...
function ViewportSceneCard({
  ...
  variant = 'section',
  delay = 0,
}: Omit<ConstellationSceneCardProps, 'sequence'>) {
